...
const APIDataPage: React.FC = () => {
  ...
  const bumpDataStatus = () => {
    const now = Date.now();
    setStatboticsRefreshKey(now);
    setDataStatusRefreshKey(now);
  };
  ...
  const handleLoadStatboticsEPA = async () => {
    ...
    executeWithConfirmation(async () => {
      ...
      try {
        // Prefer TBA team list — do not rely on match-cache toast as “EPA loaded”
        let eventTeams = await fetchEventTeamNumbersFromTBA(eventKey, apiKey);
        if (eventTeams.length === 0) {
          const validationData = await fetchValidationMatches(eventKey, apiKey, false);
          eventTeams = extractTeamsFromMatches(validationData);
        ...
        if (eventTeams.length === 0) {
          toast.error('No teams found for this event — load Event Teams or Match Validation first');
          return;
        }

        toast.info(
          `Fetching Statbotics EPA for ${eventTeams.length} teams — usually a few minutes, not instant`
        );
        const metrics = await fetchAndCacheEventStatboticsEPA(eventKey, eventTeams);
        bumpDataStatus();
  ...
  return (
    <div className="min-h-screen container mx-auto px-4 pt-12 pb-24 space-y-6 max-w-7xl">
      ...
      {/* Data Status Card */}
      <DataStatusCard eventKey={eventKey} refreshKey={dataStatusRefreshKey} />
      ...
      <DataOperationsCard
        ...
        statboticsLoading={statboticsLoading}
