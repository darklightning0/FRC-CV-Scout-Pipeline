...
import type { CvMatchTelemetryEntry } from '../types/cv-telemetry';
...
export class MatchScoutingDB extends Dexie {
  ...
  /** CV / RobotDetector telemetry — never merges into scout auto/teleop paths */
  cvMatchTelemetry!: Table<CvMatchTelemetryEntry, string>;
  ...
  constructor() {
    ...
    this.version(3).stores({
      ...
      strategySnapshots: 'id, teamNumber, eventKey, updatedAt, cacheVersion, [teamNumber+eventKey]',
      strategyCacheMetadata: 'id, version, updatedAt',
      cvMatchTelemetry: 'id, matchKey, eventKey, teamNumber, alliance, importedAt, [teamNumber+eventKey], [matchKey+teamNumber]',
    });
  }
}
...
// ============================================================================
// CV MATCH TELEMETRY (RobotDetector) — separate from human scout paths
// ============================================================================
...
export const clearCvMatchTelemetry = async (): Promise<void> => {
  await db.cvMatchTelemetry.clear();
};

export const importCvMatchTelemetryEntries = async (
  entries: CvMatchTelemetryEntry[],
  mode: 'upsert' | 'overwrite' = 'upsert'
): Promise<CvImportResult> => {
  try {
    if (mode === 'overwrite') {
      // Caller may have cleared globally; still upsert by id within this batch
      await db.cvMatchTelemetry.bulkPut(entries);
      return { success: true, importedCount: entries.length, updatedCount: 0 };
    }

    const existing = await db.cvMatchTelemetry.bulkGet(entries.map((e) => e.id));
    const existingById = new Map(
      existing
        .filter(Boolean)
        .map((e) => [(e as CvMatchTelemetryEntry).id, e as CvMatchTelemetryEntry])
    );
    const merged = entries.map((entry) => {
      const prev = existingById.get(entry.id);
      if (prev?.heatmapDataUrl && !entry.heatmapDataUrl) {
        return { ...entry, heatmapDataUrl: prev.heatmapDataUrl };
      }
      return entry;
    });
    await db.cvMatchTelemetry.bulkPut(merged);
    const updatedCount = entries.filter((e) => existingById.has(e.id)).length;
    return {
      success: true,
    ...
    console.error('CV telemetry import failed:', error);
    return {
      success: false,
      importedCount: 0,
      error: error instanceof Error ? error.message : 'Unknown error',
...
export const loadAllCvMatchTelemetry = async (): Promise<CvMatchTelemetryEntry[]> => {
  return db.cvMatchTelemetry.toArray();
};

export const loadCvMatchTelemetryByMatch = async (
  matchKey: string
): Promise<CvMatchTelemetryEntry[]> => {
  return db.cvMatchTelemetry.where('matchKey').equals(matchKey).toArray();
};

export const loadCvMatchTelemetryByTeamAndEvent = async (
  teamNumber: number,
  eventKey: string
): Promise<CvMatchTelemetryEntry[]> => {
  return db.cvMatchTelemetry
    .where('[teamNumber+eventKey]')
    .equals([teamNumber, eventKey])
    .toArray();
};

export const getCvMatchTelemetry = async (
  matchKey: string,
  teamNumber: number
): Promise<CvMatchTelemetryEntry | undefined> => {
  return db.cvMatchTelemetry.get(`${matchKey}_${teamNumber}`);
};
