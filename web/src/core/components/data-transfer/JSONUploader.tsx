...
import { handleAiScoutBundleUpload } from "@/core/lib/uploadHandlers/aiScoutBundleUploadHandler";
...
const JSONUploader: React.FC<JSONUploaderProps> = ({ onBack }) => {
  ...
  const [detectedDataType, setDetectedDataType] = useState<'scouting' | 'scoutProfiles' | 'pitScouting' | 'pitScoutingImagesOnly' | 'matchSchedule' | 'aiScoutBundle' | null>(null);
  ...
  const handleFileSelect = async (event: FileSelectEvent): Promise<void> => {
    ...
    try {
      ...
      const dataTypeNames = {
        ...
        aiScoutBundle: '🤖 AI Computer Vision Telemetry Bundle'
      };
  ...
  const handleUpload = async (mode: UploadMode): Promise<void> => {
    ...
    try {
      ...
      } else if (detectedDataType === 'aiScoutBundle') {
        await handleAiScoutBundleUpload(jsonData, mode);
      }
