...
interface DataStatusCardProps {
  eventKey: string;
  /** Bump after any load that mutates caches so status re-reads localStorage/IndexedDB */
  refreshKey?: number;
}
...
export const DataStatusCard: React.FC<DataStatusCardProps> = ({
  ...
  // refreshKey forces re-evaluation of localStorage-backed EPA on each bump
  void refreshKey;
  ...
  const statusItems: StatusItem[] = [
    ...
    },
    {
      ...
      count: hasStatboticsEPA ? statboticsTeams.size : undefined,
      details: hasStatboticsEPA
        ? `Team-event EPA cached for ${statboticsTeams.size} teams`
        : 'No team EPA data cached yet — Load Statbotics EPA (can take several minutes)',
      icon: ShieldCheck
    },
    {
